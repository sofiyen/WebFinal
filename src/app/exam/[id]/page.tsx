    ...
    <div className="flex gap-6">
      <section className="flex-1 rounded-xl border border-slate-200 bg-white p-4">
        <header className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] text-slate-400">考古題編號 #{id}</p>
          ...
          <div className="flex flex-col items-end gap-2 text-xs">
            <button
              type="button"
              className="inline-flex items-center gap-1 rounded-full bg-[#FFCB47]/10 px-3 py-1 text-[11px] font-medium text-[#b28719]"
            >
              ⚡ 128 個閃電
            </button>
        ...
        <section className="mt-4 rounded-lg border border-dashed border-slate-200 bg-slate-50/40 p-3 text-[11px] text-slate-600">
      ...
      <aside className="w-72 shrink-0 rounded-xl border border-slate-200 bg-white p-4 text-xs">
        <p className="text-[11px] font-medium text-slate-700">簡要資訊</p>
        ...
        <p className="mt-4 text-[11px] font-medium text-slate-700">
          之後會在這裡顯示：
        </p>
        <ul className="mt-1 list-disc space-y-1 pl-4 text-[11px] text-slate-500">
          <li>與此考古題相關的推薦考古題</li>
          <li>收藏到哪些資料夾</li>
          <li>實際的檔案預覽畫面</li>
        </ul>
