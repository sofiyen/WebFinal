      ...
      >
        {isEditing ? (
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div>
              <label htmlFor="edit-name" className="block text-[0.9rem] font-medium text-gray-700">
                資料夾名稱
              </label>
              <input
                type="text"
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-[0.9rem] shadow-sm focus:border-theme-color focus:outline-none focus:ring-1 focus:ring-theme-color"
              />
            </div>
            <div>
              <label htmlFor="edit-desc" className="block text-[0.9rem] font-medium text-gray-700">
                備註
              </label>
              <textarea
                id="edit-desc"
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="rounded-md bg-theme-color px-3 py-2 text-[0.9rem] font-medium text-white shadow-sm hover:bg-[#3d7a69]"
              >
                儲存
              </button>
            </div>
          ...
          <>
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                {folder.description && (
                  <p className="mt-1 text-[0.9rem] text-gray-500">{folder.description}</p>
                )}
              </div>
                  ...
                  {showMenu && (
                    <div className="absolute right-0 top-full mt-1 w-32 rounded-md border border-gray-100 bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5">
                      <button
                        ...
                        className="block w-full px-4 py-2 text-left text-[0.9rem] text-gray-700 hover:bg-gray-50"
                      >
                        編輯資料夾
                      </button>
                      <button
                        onClick={handleDelete}
                        className="block w-full px-4 py-2 text-left text-[0.9rem] text-red-600 hover:bg-red-50"
                      >
                        刪除資料夾
                      </button>
                    </div>
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              <h3 className="mb-2 text-[0.9rem] font-medium text-gray-500">
                內容 ({folder.exams.length})
              </h3>
              {folder.exams.length === 0 ? (
                <div className="rounded-lg border border-dashed border-gray-200 py-8 text-center text-[0.9rem] text-gray-400">
                  此資料夾還是空的
                  <br />
                  去逛逛考古題並加入吧！
                </div>
                ...
                <ul className="space-y-2">
                  {folder.exams.map((examId) => (
                    <li
                      key={examId}
                      className="flex items-center justify-between rounded-md bg-gray-50 px-3 py-2 text-[0.9rem]"
                    >
                      <span className="text-gray-700">考古題 ID: {examId}</span>
                      <button
                        onClick={() => handleRemoveExam(examId)}
                        className="text-[0.9rem] text-red-400 hover:text-red-600"
                      >
                        移除
                      </button>
                    </li>
